    ...
    return (
        <div className='post' style={{width:"780px"}}>
            <div className='writer' style={{display:"flex"}}>
                ...
                <div onClick={()=>{
                    navigate("/memberPage/"+props.post.writer);
                }}>{props.post.writer}&nbsp;&nbsp;</div>
                {
                    ( 
                        ( props.post.writer != lUser.nickname) &&  
                        ( !followings.some( (following)=>{
                            return following.fto == props.post.writer
                        } ) )
                    )?( <button onClick={()=>{onFollow(props.post.writer)}}>FOLLOW</button> ): (null)
                }
                {/* 객체.some((변수)=>{}) : 객체의 요소 하나하나를 한번씩 '변수'에 저장하고 익명함수를 반복 실행합니다. 
                    대개는 익명함수에서 비교연산의 결과를 리턴하는데, 그 결과가 모두 false라면 최종 결과 false이며, 리턴 값중 하나라도 
                    true 가 있으면 최종 결과는 true 입니다. 위 명령은 내가 팔로잉 하는 사람들을 현재 post 의 작성자가 있다면 true, 없다면 false 가 결과가 됩니다. 
                */}
